...
class Global_parameter extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'reference/Global_parameter?flag='.$_GET['flag'].'');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            echo 'Session Expired !'; exit;
        }
        /*load model*/
        $this->load->model('reference/Global_parameter_model', 'Global_parameter');
        /*enable profiler*/
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit '.ucwords(str_replace('_',' ',$_GET['flag'])).'', 'reference/Global_parameter/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id.'?flag='.$_GET['flag']);
            /*get value by id*/
            ...
            $this->breadcrumbs->push('Add '.ucwords(str_replace('_',' ',$_GET['flag'])).'', 'reference/Global_parameter/'.strtolower(get_class($this)).'/form');
            /*initialize flag for form add*/
            $data['flag'] = "create";
        }
        $data['flag_string'] = $_GET['flag'];
        /*title header*/
        $data['title'] = ucwords(str_replace('_',' ',$_GET['flag']));
        /*show breadcrumbs*/
    ...
    public function show($id)
    {
        ...
        $this->breadcrumbs->push('View '.ucwords(str_replace('_',' ',$_GET['flag'])).'', 'reference/Global_parameter/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id.'?flag='.$_GET['flag']);
        ...
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="center"><div class="btn-group">
                        ...
                        <ul class="dropdown-menu dropdown-inverse">
                            <li>'.$this->authuser->show_button('reference/Global_parameter?flag='.$_GET['flag'].'','R',$row_list->id,67).'</li>
                            <li>'.$this->authuser->show_button('reference/Global_parameter?flag='.$_GET['flag'].'','U',$row_list->id,67).'</li>
                            <li>'.$this->authuser->show_button('reference/Global_parameter?flag='.$_GET['flag'].'','D',$row_list->id,6).'</li>
                        </ul>
                      </div></div>';
            
            $row[] = '<div class="center">'.$row_list->id.'</div>';
            $row[] = strtoupper($row_list->label);
            $row[] = strtoupper($row_list->value);
